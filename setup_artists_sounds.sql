-- Add artists, sounds, albums, events tables to existing database
-- This script adds the missing tables without migration conflicts
...
-- Create sounds table if not exists
...
DROP POLICY IF EXISTS "Active artists are viewable by everyone" ON public.artists;
DROP POLICY IF EXISTS "Artists can update their own profile" ON public.artists;
DROP POLICY IF EXISTS "Published sounds are viewable by everyone" ON public.sounds;
DROP POLICY IF EXISTS "Artists can manage their own sounds" ON public.sounds;
DROP POLICY IF EXISTS "Published albums are viewable by everyone" ON public.albums;
DROP POLICY IF EXISTS "Artists can manage their own albums" ON public.albums;
DROP POLICY IF EXISTS "Published events are viewable by everyone" ON public.events;
DROP POLICY IF EXISTS "Artists can manage their own events" ON public.events;
...
DROP TRIGGER IF EXISTS handle_sounds_updated_at ON public.sounds;
DROP TRIGGER IF EXISTS handle_albums_updated_at ON public.albums;
